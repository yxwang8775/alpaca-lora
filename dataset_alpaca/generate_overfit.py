...
def build_instructions(template: str, data: dict):
    text = template
...
def generate_data(task_name, num, file=None, prefix=None):
    ...
    for d in tqdm(dataset):
        ...
        for template in templates:
            ...
            for t in template:
                t_full += t
            text = build_instructions(t_full, d)
            new_data.append(text)
    js = []
    for d in new_data:
        js.append({'text': d, 'task': task_name})
    if file is None:
        if prefix is None:
...
for task_name in ['qnli']:
    ...
    num = 1000000
    generate_data(task_name=task_name, num=num, prefix='overfit_qnli')
